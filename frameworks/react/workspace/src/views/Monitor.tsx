	...
	return (
		<div style={{ display: "flex", flexDirection: "column", gap: 16, padding: 16 }}>
			...
			<div style={{ fontSize: 12, color: "#666" }}>
				{lastContextAt
					? `Last context @ ${new Date(lastContextAt).toLocaleTimeString()}`
					: "Awaiting context…"}
			</div>
			...
			<div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
				<button
					type="button"
					onClick={handleBroadcastInstruments}
					disabled={!sessionContextGroup}
					title={!sessionContextGroup ? "Joining context group…" : undefined}
				>
				...
				<button
					type="button"
					onClick={() => handleSetHorizon("1 Month", "1mo")}
					disabled={!sessionContextGroup}
				>
					1M
				</button>
				<button
					type="button"
					onClick={() => handleSetHorizon("3 Months", "3mo")}
					disabled={!sessionContextGroup}
				>
					3M
				</button>
				<button
					type="button"
					onClick={() => handleSetHorizon("6 Months", "6mo")}
					disabled={!sessionContextGroup}
				>
					6M
				</button>
				<button
					type="button"
					onClick={() => handleSetHorizon("1 Year", "1y")}
					disabled={!sessionContextGroup}
				>
					1Y
				</button>
				<button
					type="button"
					onClick={() => handleSetHorizon("5 Years", "5y")}
					disabled={!sessionContextGroup}
				>
					5Y
				</button>
				<button
					type="button"
					onClick={() => handleSetHorizon("10 Years", "10y")}
					disabled={!sessionContextGroup}
				>
					10Y
				</button>
				<button
					type="button"
					onClick={() => handleSetHorizon("20 Years", "20y")}
					disabled={!sessionContextGroup}
				>
					20Y
				</button>
